...
angular.module('inspinia',['infinity.angular-chosen']).controller('activationCodeCtrl',function($scope,$http,$state,$stateParams,i18nService,$timeout){
	...
	$scope.activationCodeGrid = {
        ...
        columnDefs: [
            ...
            {field: 'action',displayName: '操作',width: 250,pinnedRight:true,sortable: false,editable:true,cellTemplate:
	            ...
	            +'<a class="lh30" ng-show="row.entity.status == 1 && grid.appScope.hasPermit(\'exchangeActivate.recovery\')" ng-click="grid.appScope.recoverySingleActivationCode(row.entity)"> | 回收</a>'
            	+'</div>'
            }
        ],
	...
	$scope.listActivationCode = function () {
		...
		$scope.loadImg = true;
        $http({
            url: 'exchangeActivationCodeAction/listActivationCode?pageNo='+$scope.page.pageNo +"&pageSize=" + $scope.page.pageSize,
            method: 'POST',
            data: $scope.info
	...
	$scope.buildActivationCode = function () {
        ...
        $http({
            url: 'exchangeActivationCodeAction/buildActivationCode?count='+$scope.buildActivation.count,
            method: 'POST'
        }).success(function (data) {
            if(data.result){
	...
	$scope.divideActivationCode = function () {
        $http({
            url: 'exchangeActivationCodeAction/divideActivationCode?startId='+$scope.divideActivation.startId
	...
	$scope.recoveryActivationCode = function () {
        $http({
            url: 'exchangeActivationCodeAction/recoveryActivation?startId='+$scope.recoveryActivation.startId
	...
	$scope.recoverySingleActivationCode = function (data) {
        $http({
            url: 'exchangeActivationCodeAction/recoveryActivation?startId='+data.id
            + "&endId="+data.id,
            method: 'POST'
        }).success(function (data) {
            if(data.result){
    ...
    $scope.ayncQueryAgentList("");
    $http({
        url: 'exchangeActivationCodeAction/getRedeemJHCode',
        method: 'POST'
    }).success(function (data) {
        $scope.activeCodeUrl.prefix = data.result && data.data && data.data.sysValue || "";
    })
    // ============= end 初始化 ====================
