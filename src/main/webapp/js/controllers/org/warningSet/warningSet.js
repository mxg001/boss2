 ...
 * 预警阀值设置
 */
angular.module('inspinia',['uiSwitch', 'infinity.angular-chosen']).controller('warningSetCtrl',function($scope, $http,$timeout,$document, i18nService,SweetAlert) {
    ...
    $scope.columnDefInfo = [
        {field: 'serviceId',displayName: '收单服务ID',width: 120,pinnable: false,sortable: false},
        {field: 'serviceName',displayName: '服务名称',width: 150},
        {field: 'acqEnname',displayName: '收单机构',width: 150},
        {field: 'serviceType',displayName: '服务类型',width: 150, cellFilter:"formatDropping:" + angular.toJson($scope.acqServiceTypes)},
        {field: 'warnTimeStr',displayName: '预警时间',width: 150},
        {field: 'warningCycleStr',displayName: '预警周期',width: 150},
        {field: 'exceptionNumber',displayName: '异常笔数',width: 150},
        {field: 'warnStatus',displayName: '状态',width: 150,cellTemplate:
            '<span ng-show="grid.appScope.hasPermit(\'warningSet.updateWarnStatus\')"><switch class="switch switch-s" ng-model="row.entity.warnStatus" ng-change="grid.appScope.switchStatus(row)" /></span>'
            +'<span class="lh30" ng-show="!grid.appScope.hasPermit(\'warningSet.updateWarnStatus\')"> <span ng-show="row.entity.warnStatus==1">打开</span><span ng-show="row.entity.warnStatus==0">关闭</span></span>'
        },
        {field: 'action',displayName: '操作',width: 150,pinnedRight:true,sortable: false,editable:true,cellTemplate:
            '<div class="lh30">'
            +'<a ng-show="grid.appScope.hasPermit(\'warningSet.updateWaringInfo\')" '
            + 'ng-click="grid.appScope.updateModal(row.entity)">修改 </a>'
            +'<a ng-show="grid.appScope.hasPermit(\'warningSet.deleteWarning\')'
...
//		rowHeight:35,
    ...
    $scope.query=function(){
        ...
        $scope.loadImg = true;
        $http({
            url: 'warningSet/selectPage?pageNo='+$scope.paginationOptions.pageNo+'&pageSize='+$scope.paginationOptions.pageSize,
            data: $scope.baseInfo,
            method:'POST'
    ...
    $scope.updateWarn = function(){
        $http.post('warningSet/updateWaringInfo', "info=" + angular.toJson($scope.warnInfo),
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    ...
                    $http({
                        method: 'POST',
                        url: 'warningSet/updateWarnStatus',
    ...
    $scope.exportInfo = function () {
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    location.href = "warningSet/exportInfo?baseInfo=" +encodeURI(encodeURI(angular.toJson($scope.baseInfo)));
                }
            });
    };
