 ...
 * 出款订单查询
 */
angular.module('inspinia').controller('hisoutOrderQueryCtrl',function($scope,$http,$state,$stateParams,$compile,$uibModal,SweetAlert,$log,i18nService,$document){
	...
	$scope.query=function(){
		...
		$scope.loadImg = true;
		$http.post("settleOrderInfoHistoryAction/getTotalNumAndTotalMoney","info="+angular.toJson($scope.info),
		...
		$http.post("settleOrderInfoHistoryAction/selectAllInfo","info="+angular.toJson($scope.info)+"&pageNo="+$scope.paginationOptions.pageNo+"&pageSize="+$scope.paginationOptions.pageSize,
				{headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
			.success(function(msg){
				if(msg.bols){
	...
	$scope.gridOptions={                           //配置表格
		      ...
		      columnDefs:[                           //表格数据
		         ...
		         { field: 'id',displayName:'操作',pinnedRight:true,width:120,
		        	 cellTemplate:'<a class="checkbox" ui-sref="histrade.hisoutOrderDetail({id:row.entity.settleOrder})" target="_black">详情</a>'
		         }
		      ],
	...
	$scope.import=function(){
		if (!($scope.info.settleUserNo || $scope.info.mobilephone || $scope.info.sourceBatchNo)) {
		 ...
		 SweetAlert.swal({
		        ...
		        function (isConfirm) {
		            if (isConfirm) {
				       	location.href="settleOrderInfoHistoryAction/exportAllInfo?info="+encodeURI(angular.toJson($scope.info));
		            }
	        });
	}
