 ...
 * 超级nfc，激活码管理
 */
angular.module('inspinia',['infinity.angular-chosen']).controller('nfcActivationCodeCtrl',function($scope,$http,$state,$stateParams,i18nService,$timeout){
	//数据源
	...
	$scope.buildActivationCode = function () {
        ...
        $http({
            url: 'activationCodeAction/buildActivationCode?count='+$scope.buildActivation.count+"&codeType="+$scope.codeType,
            method: 'POST'
        }).success(function (data) {
            if(data.result){
	...
	$scope.recoveryActivationCode = function () {
        $http({
            ...
            + "&endId="+$scope.recoveryActivation.endId + "&codeType=" + $scope.codeType,
            method: 'POST'
        }).success(function (data) {
            if(data.result){
	...
	$scope.recoverySingleActivationCode = function (data) {
        $http({
            ...
            + "&endId="+data.id+ "&codeType=" + $scope.codeType,
            method: 'POST'
        }).success(function (data) {
            if(data.result){
                $scope.notice("操作成功");
