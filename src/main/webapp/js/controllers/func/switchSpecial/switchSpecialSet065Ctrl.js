 ...
 * 065开关特殊配置
 */
angular.module('inspinia').controller('switchSpecialSet065Ctrl',function(i18nService,$scope,$http,$state,$stateParams,$compile,$filter,SweetAlert){
	...
	$scope.teamGrid={                           //配置表格
		...
		columnDefs:[                           //表格数据
			{field: 'appName', displayName: '产品APP名称'},
			...
			{ field: 'id',displayName:'操作',width:150,pinnedRight:true, cellTemplate:
					'<div class="lh30">'+
						'<a ng-show="grid.appScope.hasPermit(\'func.saveSettingList\')" ui-sref="func.appShowConfig({fmcId:row.entity.id})">管理</a> ' +
	...
	$scope.appInfoList=[];
	//appInfo
	$http.get('appInfoAction/getAllAppInfo').success(function(msg){
		var list=msg.list;
		$scope.appInfoList.push({text:"请选择",value:null});
		if(list!=null&&list.length>0){
			for(var i=0;i<list.length;i++){
				$scope.appInfoList.push({text:list[i].appName,value:list[i].appNo+","+list[i].appName+","+list[i].teamId});
			}
		}
	});
	...
	$scope.appNoChange = function(appNo){
		if(appNo == "" || appNo == null){
			$scope.addInfo.teamId = null;
		}else {
			$scope.addInfo.teamId = appNo.split(",")[2];
			//转换值
			angular.forEach($scope.teamType, function (e) {
				if(e.value!=null&&e.value.length>0){
					var item=e.value.split(",")[0];
					if($scope.addInfo.teamId==item){
						$scope.addInfo.teamId=e.value;
					}
				}
			});
		}
	...
	$scope.teamAddModalShow = function(){
		$scope.addInfo = {appNo:null,teamId:null,teamEntryId:null};
		$scope.subTeams = [];
	...
	$scope.teamAdd=function () {
		...
		//数据转换
		if($scope.subInfo.appNo!=null&&$scope.subInfo.appNo!=""){
			var strs=$scope.subInfo.appNo.split(",");
			$scope.subInfo.appNo=strs[0];
			$scope.subInfo.appName=strs[1];
		}
