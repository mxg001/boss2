angular.module('inspinia',['uiSwitch']).controller('switchSpecialSet074Ctrl',function(i18nService,$scope,$http,$state,$stateParams,$compile,$filter,SweetAlert){
    ...
    $scope.teamGrid={                           //配置表格
        ...
        columnDefs:[                           //表格数据
            {field: 'teamName', displayName: '组织',width:120},
            {field: 'teamId', displayName: '组织ID',width:120},
            {field: 'teamEntryName', displayName: '子组织',width:120},
            {field: 'teamEntryId', displayName: '子组织ID',width:120},
            {field: 'effectiveTime',displayName:'生效时间',cellFilter: 'date:"yyyy-MM-dd HH:mm:ss"',width:180},
            {field: 'invalidTime',displayName:'失效时间',cellFilter: 'date:"yyyy-MM-dd HH:mm:ss"',width:180},
            {field: 'actLimitDay', displayName: '激活天数限制',width:120},
            { field: 'id',displayName:'操作',width:150,pinnedRight:true, cellTemplate:
                ...
                '<a ng-click="grid.appScope.deleteTeamInfo(row.entity.id)">删除</a> ' +
                '</div>'
            }
        ],
    ...
    $scope.teamEditModalShow = function(id){
        $http.post("functionManager/getTeamModelInfo","functionNumber=074&id="+id,
    ...
    $scope.teamAdd=function () {
        ...
        if($scope.subInfo.effectiveTime==""){
            $scope.notice("生效时间不能为空");
            $scope.submittingMode=false;
            return;
        }
        if($scope.subInfo.invalidTime==""){
            $scope.notice("失效时间不能为空");
            $scope.submittingMode=false;
            return;
        }
        if($scope.subInfo.effectiveTime>$scope.subInfo.invalidTime){
            $scope.notice("生效时间不能大于失效时间");
            $scope.submittingMode=false;
            return;
        }
        if(!isPositiveNum($scope.subInfo.actLimitDay)){
            $scope.notice("激活天数限制必须是正整数");
            $scope.submittingMode=false;
            return;
        }
        ...
        if($scope.subInfo.actLimitDay<0){
