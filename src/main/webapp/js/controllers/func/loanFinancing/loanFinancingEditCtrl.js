 ...
 * 贷款理财配置修改
 */
angular.module('inspinia',['angularFileUpload']).controller('loanFinancingEditCtrl',function($scope,$http,i18nService,$state,FileUploader,SweetAlert,$stateParams){
    i18nService.setCurrentLang('zh-cn');  //设置语言为中文

    $scope.addInfo={};
    $http.post("loanFinancing/getLoanFinancing","id="+$stateParams.id,
    ...
    $scope.subAddInfoPost=function (url) {
        ...
        $http.post("loanFinancing/updateLoanFinancing",data,postCfg)
            .success(function(data){
                $scope.submitting = false;
                if(data.status){
                    $scope.notice(data.msg);
