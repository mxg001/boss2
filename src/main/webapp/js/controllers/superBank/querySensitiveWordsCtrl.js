...
angular.module('inspinia').controller("querySensitiveWordsCtrl", function($scope, $http, $state, $stateParams,$filter,i18nService,SweetAlert,$document) {
    ...
    $scope.queryFunc = function(){
        $scope.result=[];
        $http.post("superSensitiveWords/selectByParam.do","baseInfo=" + angular.toJson($scope.baseInfo)+"&pageNo="+
    ...
    $scope.sensitiveListGrid = {
        ...
        columnDefs: [
            ...
            '<div class="lh30">' +
            '<a ng-show="grid.appScope.hasPermit(\'superSensitiveWords.deleteSensitiveWords\')" ng-click="grid.appScope.deleteSensitiveWords(row.entity.id)">删除</a>'+
            '</div>'}
        ],
    ...
    $scope.deleteSensitiveWords = function(id){
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.get('superSensitiveWords/deleteSensitiveWords/'+id)
    ...
    $scope.deleteSensitiveWordsAll = function(id){
        ...
        SweetAlert.swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $http.post("superSensitiveWords/batchDeleteSensitiveWords","swlist=" + angular.toJson(list),
                        {headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
                        .success(function(msg){
                            $scope.notice(msg.msg);
                            $scope.queryFunc();
    ...
    $scope.addSensitiveWords=function(){
        ...
        $http.post("superSensitiveWords/saveSensitiveWords","baseInfo=" + angular.toJson($scope.info),
            {headers: {'Content-Type':'application/x-www-form-urlencoded'}})
            .success(function(data){
                if(data.status){
