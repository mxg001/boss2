 ...
 * 贷款机构配置
 */
angular.module('inspinia',['uiSwitch']).controller('orgLoansConfigCtrl',function($scope,$http,i18nService,$stateParams,$document,SweetAlert){
    ...
    $scope.resetForm1 = function () {
        $scope.baseInfo1 = {type:'2',status:'',"orgId":$stateParams.orgId,"sourceId":""};//查询条件的form
    };
    $scope.resetForm1();
    $scope.resetForm2 = function () {
        $scope.baseInfo2 = {type:'2',status:'',"orgId":$stateParams.orgId,"sourceId":""};//查询条件的form
    };
    $scope.resetForm2();
    $scope.gongzhonghaoColumnDefs = [
                         {field: 'code',width: 120, displayName: '贷款机构编码',pinnable: false,sortable: false},
                         {field: 'sourceName',width: 150, displayName: '贷款机构全称',pinnable: false,sortable: false},
                         {field: 'sourceNickName',width: 150, displayName: '贷款机构别称',pinnable: false,sortable: false},
        				...
        				/*{field: 'specialImage',displayName: '特别推荐图片',width: 150,pinnable: false,sortable: false, cellTemplate:'<img style="width: 140px; height: 36px;" ng-show="row.entity.specialImageUrl" ng-src="{{row.entity.specialImageUrl}}" />'},*/
                         {field: 'statusInt',displayName: '是否上架',width:150,pinnable: false,sortable: false,cellTemplate:
                             '<span ng-show="grid.appScope.hasPermit(\'superBank.orgLoansConfig\')">' +
    ...
    $scope.appColumnDefs = [
      ...
      //批量上下架
    	...
    	$scope.getAllLoans= function(){
        	$http({
                url: "orgSourceConfig/loansList",
                method: "POST"
            }).success(function(result){
            	 $scope.loans = result.data;
            	 $scope.loans.unshift({id:"",loanAlias:"全部"});
            });
        };
        ...
        $scope.gongzhonghaoSaveUpd = function(){
        	...
        	//$scope.rows = $scope.gridApi.selection.getSelectedGridRows();
        	var reg = /^\d{1,}$/;
        	$scope.isTrue = true;
        	$scope.data = "[";
        	angular.forEach($scope.rows, function(data,index,array){
      ...
      //保存更改
        $scope.appSaveUpd = function(){
        	...
        	//$scope.rows = $scope.gridApi2.selection.getSelectedGridRows();
        	var reg = /^\d{1,}$/;
        	$scope.isTrue = true;
        	$scope.data = "[";
        	angular.forEach($scope.rows, function(data,index,array){
