 ...
 *
 */
function MainCtrl($scope,notify,$http,$interval,$state) {
	/**
	 * 常量
	 */
    ...
    // $scope.timeout="";
    $scope.exportList=[];
    $scope.exportListSize=0;
    $scope.oldexportList=[];
    $scope.exportManageID=-1;
    $scope.windowSta=false;
    $scope.checkList=function(){
        ...
        for(var i=0;i<$scope.exportList.length;i++){
            var sta=1;
            if($scope.oldexportList.length>0){
                for(var j=0;j<$scope.oldexportList.length;j++){
                    if($scope.exportList[i].id==$scope.oldexportList[j].id){
                        sta=0;
                        break;
                    }
                }
    ...
    $scope.getExportManageTime=function(checkSta){
        $http.post("exportManage/selectAllListTime",{headers: {'Content-Type': 'application/x-www-form-urlencoded'}})
            .success(function(data){
                if(data.status){
                    $scope.exportList=data.page.result;
                    $scope.exportListSize = data.page.totalCount;
                    if(checkSta){
                        $scope.checkList();
                    }
                    $scope.oldexportList=angular.copy($scope.exportList);
                }
            });
    };
    $scope.getExportManageTime(false);
    ...
    //            $scope.getExportManageTime(true);
    //        },30000);//30秒一次 30000
    //    }
    // };
    // $scope.timeInit();
    ...
    //点击标记已读
    $scope.exportManageClick=function(id){
        $http.post("exportManage/exportManageClick","id="+id,
            ...
            .success(function(data){
                if(data.status){
                    ...
                    $scope.exportListSize = data.page.totalCount;
                    //$scope.oldexportList=angular.copy($scope.exportList);
                    $scope.exportManageID=id;
                    $state.transitionTo('sys.exportManage',null,{reload:true});
                }else{
    ...
    function getContextPath() {
        // 获取主机地址之后的目录，如： /boss/index.html
        var pathName = window.document.location.pathname;
        // 获取带"/"的项目名，如：/MyWeb
        var projectName = pathName.substring(0, pathName.substr(1).indexOf('/') + 1);
        return projectName;
    }
    /* 获取webSocket需要的路径格式 如何 localhost:8080,localhost:8080/JfBoss */
    ...
    var httpurl = window.document.location.href;

    if(httpurl.indexOf('https')>=0){
        console.log("https模式");
        if ('WebSocket' in window) {
            websocket = new WebSocket("wss://" + projectUrl + "/websocket/socketServer");
        } else if ('MozWebSocket' in window) {
            websocket = new MozWebSocket("wss://" + projectUrl + "/websocket/socketServer");
        } else {
            websocket = new SockJS("https://" + projectUrl + "/sockjs/socketServer");
        }
    }else{
        console.log("http模式");
        if ('WebSocket' in window) {
            websocket = new WebSocket("ws://" + projectUrl + "/websocket/socketServer");
        } else if ('MozWebSocket' in window) {
            websocket = new MozWebSocket("ws://" + projectUrl + "/websocket/socketServer");
        } else {
            websocket = new SockJS("http://" + projectUrl + "/sockjs/socketServer");
        }
    }
	...
	$scope.cancelVerType = [{text:"全部",value:''},{text:'扣减交易手续费',value:'2'},{text:'扣减提现手续费',value:'1'},{text:'过期',value:'3'},{text:'抵扣优享(二)手续费',value:'4'}];
...
//	$scope.agentLevelAll.unshift({text:"全部",value:""});
    ...
    //新欢乐送代理商奖励累计交易统计方式
    $scope.transTotalTypes= angular.copy(initData.XHLS_TRANS_TOTAL_TYPES);

    //投诉下发验证码-短信发送部门列表
    $scope.departmentTypeList = angular.copy(initData.DEPARTMENT_TYPE);
    //业务类型
    $scope.smsBusinessTypeList = angular.copy(initData.SMS_BUSINESS_TYPE);

    //app模块类型
    $scope.functionModularTypeList = angular.copy(initData.FUNCTION_MODULAR_TYPE);

    //风控规则指令
    $scope.rulesInstructions= angular.copy(initData.RULES_INSTRUCTIONS);
    $scope.rulesInstructions.unshift({text:"全部",value:"-1"});

    //券类型
    $scope.couponTypeList= angular.copy(initData.COUPON_TYPE);
  ...
  $scope.noticeHtml=function(message,options){
    var option={
      message: '测试提示！',
      classes: 'alert-info',
      templateUrl: "views/common/notifyHtml.html",
      position : "center",
      duration:5000
    };
    if (typeof options !== 'object'){
